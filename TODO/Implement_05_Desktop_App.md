...
- [x] **Sub-phase 05.1.1:** Implement `main.ts` with custom titlebar, frameless window support, and hardware acceleration flags (`--enable-gpu-rasterization`, `--enable-zero-copy`).
- [x] **Sub-phase 05.1.2:** Configure auto-reload and HMR bridge in development mode.

### Phase 05.2: Bundled FFmpeg Manager (`src/main/ffmpeg-manager.ts`)
- [x] **Sub-phase 05.2.1:** Detect bundled `ffmpeg.exe` in `resources/bin` or system PATH.
- [x] **Sub-phase 05.2.2:** Expose high-speed streaming pipe handler from Renderer process to FFmpeg `stdin`.

### Phase 05.3: Project File System & Native Dialogs (`src/main/file-manager.ts`)
- [x] **Sub-phase 05.3.1:** Implement `.focal` project package format (ZIP bundle containing `project.json`, `events.json`, and raw video/frame assets).
- [x] **Sub-phase 05.3.2:** Implement native file pickers: Open Project, Save Project As, Import Video, Export Video.

### Phase 05.4: Local WebSocket Telemetry Server (`src/main/telemetry-server.ts`)
- [x] **Sub-phase 05.4.1:** Spin up a lightweight local WebSocket server (`ws://127.0.0.1:48480`) to receive live events streamed from the Chrome Extension (Mode A).
- [x] **Sub-phase 05.4.2:** Buffer incoming events and synchronize with Electron screen recording stream.

### Phase 05.5: Secure Preload Bridge (`src/preload/`)
- [x] **Sub-phase 05.5.1:** Expose type-safe `window.focalApi`:
  ```typescript
...
- [x] **Sub-phase 05.6.1:** Configure `electron-builder` to produce Windows portable `.exe` and NSIS installer with bundled FFmpeg.
